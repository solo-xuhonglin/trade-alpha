...
API ────────────────────────────────────────────────┤
  ...
  (serialize / create / update 全自动适配)           │
                                                    │
回测快照 ───────────────────────────────────────────┤
  dao/execution.py               ← 加字段           │
  (StrategySnapshotEmbed 用于回测结果回显)            │
                                                    │
Frontend ───────────────────────────────────────────┤
  api/strategyConfig.ts          ← TS 接口加字段    │
                                                    │
  views/StrategyConfigView.vue   ← 5 处改动         │
  ├─ form 默认值                 ← 加初始值         │
  ├─ openDialog()                 ← 从 API 取值      │
  ├─ saveStrategy()               ← 发给后端         │
  ├─ compareFields               ← 加对比条目       │
...
**按顺序改**：DAO → Service → API Schema → (Router 自动) → 快照模型 → Frontend → 业务逻辑
...
| Frontend compareFields | 加对比条目（key + label + group + type） | **需要** |
| Backtest snapshot `StrategySnapshotEmbed` | 在 `dao/execution.py` 加字段（自动回显到回测结果） | **需要** |

后端只需改 **DAO + 2 个 Service 签名 + 2 个 Schema + 快照模型**，Router 和 update body 自动适配。

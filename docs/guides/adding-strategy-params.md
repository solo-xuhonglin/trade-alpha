...
## 二、已实施的 3 个优化（`87e1b11`）

三个优化均已实施并提交，共减少 415 行重复代码。
...
|--------|:-------:|:------:|:------------------:|
| Router `_strategy_to_dict` | 74 行 | 4 行 | **不需要** ✅ |
| Router create handler | ~70 行传参 | 1 行 | **不需要** ✅ |
| Router update handler | ~70 行传参 | 1 行 | **不需要** ✅ |
...
```
DAO加字段 → Service加参数×2 → Schema加字段×2 
→ (router/create/update/update body 全自动) → 前端TS+form+openDialog+save+template×5
= 9 处
```

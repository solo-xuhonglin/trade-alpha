...
### 1. 交易日期（Trade Date）- `YYYYMMDD`（无横线）

**使用场景**：
- 所有请求参数中用于查询股票数据的日期范围（`start_date`, `end_date`）
- 响应中的交易日期字段（`trade_date`）
- 预测数据的日期范围字段（`start_date`, `end_date`，用于传递给 data API）
...
**验证器**：
- `TradeDateQuery` - 用于查询参数验证
- `validate_trade_date()` - 用于请求体验证

### 2. 显示日期（Display Date）- `YYYY-MM-DD`（有横线）

**使用场景**：
- 列表显示给用户的日期范围（训练记录、回测记录、数据分析记录等）
...
后端内部自动处理日期格式转换：
- 数据库存储格式：`YYYYMMDD`
- 验证：所有交易日期请求参数会自动验证格式，格式错误时返回 `400` 错误而非 `500`

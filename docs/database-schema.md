...
| `stock_list_history` | `StockListHistory` | 历史市值快照（周度候选池数据源） |
...
|------|------|------|------|
| 主键 | `_id` | 是 | MongoDB 默认 |
| 股票+日期 | `ts_code`, `trade_date` | 否 | 复合索引，加速历史查询 |
| 日期查询 | `trade_date` | 否 | 加速按日期查询 |
...
| `is_active_for_backtest` | bool | 是否纳入回测同步范围（默认 false，可手动开关） |
...
|------|------|------|
| `name` | string | 名称 |
| `type` | string | 策略类型（single/multi） |
| `max_hold_days` | int | 最大持仓天数 |
| `min_hold_days` | int | 最低持有天数 |
| `stop_loss_pct` | float | 止损比例 |
| `atr_stop_multiplier` | float | ATR 止损乘数（默认 3.0，结合 ATR 动态止损） |
| `atr_trail_rate` | float | ATR 上移比例（默认 0.5，止损线上移速率） |
| `buy_threshold` | float | 买入评分阈值 |
| `sell_threshold` | float | 卖出评分阈值 |
| `sell_rank_pct` | float | 卖出排名百分比（默认 0.15） |
| `hold_score_threshold` | float | 持仓评分阈值 |
| `max_positions` | int | 最大持仓数（组合模式，默认 6） |
| `max_position_pct` | float | 单只股票最大资金占比（组合模式，默认 0.2） |
| `min_order_value` | float | 最小交易金额（默认 50000） |
| `max_daily_buys` | int | 每日最多买入数量（默认 2） |
| `ranking_smooth_window` | int | 排名平滑窗口 |
| `ranking_smooth_alpha` | float | 排名平滑系数 |
| `score_decline_threshold` | float | 评分下降阈值（默认 0.05） |
| `use_score_decline_filter` | bool | 是否启用评分下降过滤（默认 false） |
| `use_rank_up_priority` | bool | 排名上涨优先买入（默认 false） |
| `rank_up_window` | int | 排名均值窗口（默认 3） |
| `rank_up_count` | int | 优先买入数（默认 1） |
| `rank_up_min_score` | float | 最低评分门槛（默认 -0.1） |
| `rank_up_min_improvement_pct` | float | 最小提升比例（默认 0.15） |
| `market_smooth_window` | int | 市场分析平滑窗口（默认 3） |
| `market_smooth_alpha` | float | 市场分析平滑系数 |
| `top_n_retention_pct` | float | 留存率排名百分比（默认 0.20） |
| `retention_days` | int | 留存天数 |
| `correlation_window` | int | 评分-收益关联度窗口 |
| `use_phase_strategy` | bool | 启用市场阶段策略（默认 true） |
| `full_position_pnl_weight` | float | 满仓 PnL 权重（默认 0.5） |
| `rotation_bottom_pct` | float | 轮动回调最低排名百分比（默认 0.60） |
| `rotation_rank_min_pct` | float | 轮动买入最小排名百分比（默认 0.30） |
| `rotation_rank_max_pct` | float | 轮动买入最大排名百分比（默认 0.70） |
| `rotation_use_reversal_check` | bool | 轮动反转确认开关（默认 true） |
| `rotation_was_top_pct` | float | 轮动历史最高排名百分比（默认 0.15） |
| `rotation_was_top_window` | int | 轮动历史检测窗口（默认 60 天） |
| `rotation_pullback_window` | int | 轮动回调检测窗口（默认 5 天） |
...
| 变更类型 | 旧字段 | 新字段 | 说明 |
|---------|--------|--------|------|
| 删除 | `phase_crash_threshold` | — | 原急跌阈值，相位策略已移除 |
| 删除 | `phase_recovery_threshold` | — | 原企稳阈值，相位策略已移除 |
| 删除 | `baseline_vol_window` | — | 原基线波动窗口 |
| 删除 | `baseline_vol_ref_multiplier` | — | 原基线波动乘数 |
| 新增 | — | `atr_stop_multiplier` | ATR 动态止损乘数 |
| 新增 | — | `atr_trail_rate` | ATR 止损线上移比例 |
| 新增 | — | `max_daily_buys` | 每日买入次数限制 |
| 新增 | — | `score_decline_threshold` | 评分下降检测阈值 |
| 新增 | — | `use_score_decline_filter` | 评分下降过滤开关 |
| 新增 | — | `full_position_pnl_weight` | 满仓 PnL 计算权重 |
| 重命名 | `rotation_reversal_window` | `rotation_was_top_window` | 语义变为历史检测窗口 |
| 替换 | `sell_rank_n` | `sell_rank_pct` | 整数排名改为百分比 |
| 替换 | `rotation_bottom_threshold` | `rotation_bottom_pct` | 整数排名改为百分比 |
| 替换 | `rotation_rank_min` | `rotation_rank_min_pct` | 整数排名改为百分比 |
| 替换 | `rotation_rank_max` | `rotation_rank_max_pct` | 整数排名改为百分比 |
| 替换 | `rotation_was_top_n` | `rotation_was_top_pct` | 整数排名改为百分比 |
| 替换 | `top_n_retention` | `top_n_retention_pct` | 整数排名改为百分比 |
| 更新 | `min_order_value` | — | 默认值 5000 → 50000 |
| 更新 | `max_hold_days` | — | 默认值 120 → 180 |
| 更新 | `buy_threshold` | — | 默认值 0.2 → 0.3 |
| 更新 | `sell_threshold` | — | 默认值 -0.01 → -0.05 |
| 更新 | `max_positions` | — | 默认值 10 → 6 |
| 更新 | `max_position_pct` | — | 默认值 0.1 → 0.2 |
| 更新 | `full_position_score_window` | — | 默认值 10 → 15 |
| 更新 | `rank_up_window` | — | 默认值 5 → 3 |
| 更新 | `rank_up_count` | — | 默认值 3 → 1 |
| 更新 | `rank_up_min_score` | — | 默认值 0.1 → -0.1 |
| 更新 | `rank_up_min_improvement_pct` | — | 默认值 0.20 → 0.15 |
| 更新 | `market_smooth_window` | — | 默认值 5 → 3 |
| 更新 | `rotation_was_top_window` | — | 默认值 30 → 60 |
| 新增 | — | `use_momentum_penalty` | 动量扣分开关 |
| 新增 | — | `use_trend_penalty` | 趋势扣分开关 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `backtest_id` | ObjectId | 关联执行结果 ID |
| `trade_date` | string | 交易日期（YYYYMMDD） |
| `cash` | float | 现金余额 |
| `total_value` | float | 总资产（现金+持仓市值） |
| `day_return` | float | 当日收益率 |
| `position_pct` | float | 仓位比例 |
| `market_phase` | string | 市场阶段标识（up/flat/down） |
| `daily_rebalanced_cum` | float | 每日再平衡累计净值 |
| `rebalanced_ma10_pct` | float | 每日再平衡 10 日均线偏离度 |
| `rebalanced_ma60_pct` | float | 每日再平衡 60 日均线偏离度 |
| `ranking_high_pct` | float | 排名高分位比例 |
| `ranking_low_pct` | float | 排名低分位比例 |
| `top_n_retention_rate_smoothed` | float | 前 N 留存率 EWMA 平滑值 |
| `score_return_corr_smoothed` | float | 评分-收益关联度 EWMA 平滑值 |
| `baseline_vol_multiplier` | float | 基线波动乘数 |
...
| `stock_name` | string | 股票名称 |
| `score` | float | 基础得分 |
| `composite_score` | float | 综合评分（score + trend_bonus - trend_penalty - vol_penalty + momentum_bonus - momentum_penalty） |
...
| `trend_penalty` | float | 趋势扣分 |
| `vol_penalty` | float | 波动扣分 |
| `momentum_bonus` | float | 动量加分 |
| `momentum_penalty` | float | 动量扣分 |
...
| `atr_stop_multiplier` | 3.0 | ATR 止损乘数 |
| `atr_trail_rate` | 0.5 | ATR 上移比例 |
| `max_daily_buys` | 2 | 每日最多买入 |

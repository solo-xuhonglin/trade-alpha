...
| `rank_up_window` | int | 排名均值窗口（默认 5） |
| `rank_up_count` | int | 优先买入数（默认 3） |
| `rank_up_min_score` | float | 最低评分门槛（默认 0.1） |
| `rank_up_min_improvement_pct` | float | 最小提升比例（默认 0.20） |
| `market_smooth_window` | int | 市场分析平滑窗口 |
| `market_smooth_alpha` | float | 市场分析平滑系数 |
| `top_n_retention` | int | 留存率计算前 N 只（默认 20） |
...
| `rotation_bottom_threshold` | int | 轮动回调最低排名阈值（默认 60） |
| `rotation_rank_min` | int | 轮动排名上限（默认 45） |
| `rotation_rank_max` | int | 轮动排名下限（默认 75） |
| `rotation_use_reversal_check` | bool | 轮动反转确认开关（默认 true） |
| `rotation_was_top_n` | int | 轮动历史最高排名（默认 15） |
| `rotation_was_top_window` | int | 轮动历史检测窗口（默认 30 天） |

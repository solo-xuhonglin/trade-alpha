...
原始评分 → 原始分 (raw_score)
    ↓
趋势加分 (trend_bonus) - 趋势扣分 (trend_penalty)  波动扣分 (vol_penalty)  动量加成 (momentum_bonus) - 动量扣分 (momentum_penalty)
    ↓
...
```
原始分 → 趋势加分/扣分 → 波动扣分 → 动量加成/扣分 → 综合分 → 排名平滑 → 排名分
```
...
| `acceleration_window` | int | 5 | 检测窗口天数 |
| `acceleration_cum_return` | float | 0.25 | 累计涨幅阈值（25%） |
| `acceleration_up_ratio` | float | 0.80 | 上涨天数占比阈值（80%） |

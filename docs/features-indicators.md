...
- **ranking_median_smoothed**：EWMA 平滑后的中位数，用于市场状态分类（trending_up / trending_down / sideways）和仓位缩放因子 `score_scalar` 计算

### 排名前N留存率 (Top-N Retention Rate)

t-1日排名前 n 的股票中，t日仍然在前 n 名的比例。衡量市场持续性，越高说明强者恒强效应越明显。
...
t-1日所有股票 `composite_score` 与 t-1日实际收益率 `pct_chg` 的 Pearson 截面相关系数（排除 `is_excluded` 的股票）。衡量评分预测能力的有效性。

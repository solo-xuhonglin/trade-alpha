...
```
原始分 → 趋势加分/扣分 → 动量加成/扣分 → 评分下滑过滤 → 综合分 → 排名平滑 → 排名分
```
...
### 4. 评分下滑过滤（Score Decline Filter）

基于连续评分趋势的下滑过滤。

**算法**：检查最近两次记录的 `composite_score`，如果当天评分相比上次下降超过阈值，则标记为过滤
- 条件：当天评分 < 上次评分 - `score_decline_threshold`
- 仅当 `use_score_decline_filter` 启用时生效
- 需要至少 2 次评分记录才做判断

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `use_score_decline_filter` | bool | false | 是否启用评分下滑过滤 |
| `score_decline_threshold` | float | 0.05 | 评分下滑阈值（绝对值） |
...
`MarketRegimeAnalyzer` 在每日决策前计算全市场股票池的统计指标，经 EWMA 平滑后用于市场状态分类和仓位缩放。

### 排名高低分占比 (ranking_high_pct / ranking_low_pct)

全市场 `ranking_score` 的高分（>0.30）和低分（<-0.30）占比，反映市场评分分布情况。

- **ranking_high_pct**：当日高分占比（%）
- **ranking_low_pct**：当日低分占比（%）
...
- **rebalanced_ma10_pct**：基线 EMA 的 10 日均线偏离百分比
- **rebalanced_ma60_pct**：基线 EMA 的 60 日均线偏离百分比
- **market_phase**：市场阶段，基于 EMA 平滑后的基线指数判断：
  - `up`（price > MA60 + 3% 且 MA10 > MA60，或 price > MA60 + 1% 且 MA10/MA60 偏差 > 0.5%）
  - `down`（price < MA60 - 3% 且 MA10 < MA60，或 price < MA60 - 1% 且 MA10/MA60 偏差 < -0.5%）
  - `flat`（其他情况）
- **baseline_vol_multiplier**：基线波动乘数，通过 EMA 短期波动 / 长期波动比率计算，范围 [0.5, 3.0]
...
| `use_phase_strategy` | bool | true | 是否启用市场阶段检测 |
...
| `full_position_days` | int | 5 | 连续触发天数 |
| `full_position_score_window` | int | 10 | 评分均值窗口 |
| `full_position_sell_count` | int | 1 | 每次卖出数量 |
| `full_position_pnl_weight` | float | 0.5 | 卖出优先级的盈亏权重（评分权重=1，与评分的加权和决定卖出顺序） |

### 2. 排名上涨优先（Rank-Up Priority Buy）
...
### 3. ATR 动态追踪止损（ATR Dynamic Trailing Stop）

基于 ATR（平均真实波幅）的动态止损，随股价上涨自动上移止损线。

**算法**：
- 买入时记录初始 ATR 值和买入价格
- 初始止损价 = 买入价 - `atr_stop_multiplier` × ATR
- 每日检查：如果当前最高价 > 持仓最高价，则上移止损价
  - 新止损价 = max(旧止损价, 当前最高价 - `atr_stop_multiplier` × ATR × (1 - `atr_trail_rate`))
- 跌破止损价时触发卖出

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `atr_stop_multiplier` | float | 3.0 | ATR 止损倍数（越高越宽松） |
| `atr_trail_rate` | float | 0.5 | 追踪速率（0=固定止损，1=全追踪） |

### 4. 每日买入上限（Max Daily Buys）

限制每日最大买入股票数量，控制单日建仓节奏。

**算法**：
- 每日买入股票不超过 `max_daily_buys` 只
- 超出部分不进入买入队列
- 适用于所有买入模式（排名上涨优先和常规买入共享此限额）

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `max_daily_buys` | int | 2 | 每日最大买入股票数量 |

...
| down | 轻仓防御 | 0.5 | 改進：不买入，放宽卖出条件 |

## 2. 代码组织

```
strategy/
├── base.py                          # PositionManager（不碰）
...
2. 评分 < sell_threshold (-0.01) → SCORE_BELOW
3. 超 max_hold_days → MAX_HOLD
4. 止损 → STOP_LOSS
5. 不在前 N 名且评分 < hold_score_threshold → HOLD_SCORE_LOW
...
- `max_position_pct = 0.1 × 0.5 = 0.05`（通过 `pos_mult=0.5` 实现）
...
```python
def _check_sell_mr(self, position, close_prices, score_manager, stop_loss_pct, max_hold_days):
    ...
    # 止损（通用）
    if self._strategy._is_stop_loss_triggered(position, close_prices, stop_loss_pct):
...
| `strategy/multi_stock_strategy.py` | 修改：接入 mode 调度 + 保留通用工具方法 |

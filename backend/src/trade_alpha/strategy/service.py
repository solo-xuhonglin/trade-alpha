...
from trade_alpha.dao import Strategy, StockDaily, Prediction, Signal
...
async def create_strategy(
    ...
    existing = await Strategy.find_one(Strategy.name == name)
    if existing:
...
async def get_strategy_by_id(strategy_id: PydanticObjectId) -> Optional[Strategy]:
    """Get strategy by ID."""
    return await Strategy.get(strategy_id)


async def get_strategy_by_name(name: str) -> Optional[Strategy]:
    """Get strategy by name."""
    return await Strategy.find_one(Strategy.name == name)


async def list_strategies() -> List[Strategy]:
    """List all strategies."""
    return await Strategy.find_all().to_list()
...
async def generate_signal(
    ...
    prediction = {}
    pred_record = await Prediction.find(

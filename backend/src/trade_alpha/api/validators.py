...
def validate_date(v: Optional[str]) -> Optional[str]:
    ...
    raise HTTPException(
        status_code=400,
        detail=f"Invalid date format: '{v}'. Expected YYYY-MM-DD or YYYYMMDD"
    )


def validate_trade_date(v: Optional[str]) -> Optional[str]:
    """验证交易日期格式 (trade_date)，仅支持 YYYYMMDD（股票交易日期格式）"""
    if v is None:
        return v
    ...
    raise HTTPException(
        status_code=400,
        detail=f"Invalid trade date format: '{v}'. Trade date (trade_date) must be in YYYYMMDD format"
    )
...
"""交易日期查询参数 (trade_date)，仅支持 YYYYMMDD"""